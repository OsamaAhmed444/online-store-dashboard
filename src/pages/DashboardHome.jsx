...
import { calculateProductSales, filterTeamOrders, filterTeamProducts, getOrdersFromResponse, getProductsFromResponse, getUsersFromResponse, normalizeOrder, normalizeProduct } from '../components/dashboard/dashboardData'

const emptyDashboard = { orders: [], products: [], categoryProducts: [], users: [], topProducts: [] }
...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    Promise.all([getOrders(), getOrdersAdmin(), getProducts(), getUsers()])
      .then(([dashboardResponse, ordersResponse, productsResponse, usersResponse]) => {
        if (!mounted) return
        const rawProducts = productsResponse.data ? getProductsFromResponse(productsResponse) : []
        const teamProducts = filterTeamProducts(rawProducts)
        const orders = filterTeamOrders(mergeOrders([dashboardResponse, ordersResponse]), rawProducts)
        const products = teamProducts.map(normalizeProduct)
        setDashboard({ orders, products, categoryProducts: teamProducts, users: getUsersFromResponse(usersResponse), topProducts: calculateProductSales(orders, products) })
      })
  ...
  return <div className="dashboard-page"><section className="hero-panel"><div className="hero-copy"><p className="eyebrow">ADMIN OVERVIEW</p><h1>Real-time commerce health</h1><p>Monitor your store performance with real-time data and insights.</p></div><div className="hero-glow hero-glow-one" /><div className="hero-glow hero-glow-two" /></section>{loading ? <LoadingScreen text="Loading dashboard data..." /> : error ? <div className="dashboard-error">{error}</div> : <><section className="stats-grid" aria-label="Store statistics">{stats.map((stat) => <StatCard key={stat.title} {...stat} />)}</section><div className="dashboard-insights-row"><TopCategoriesDonut products={dashboard.categoryProducts} topProducts={dashboard.topProducts} /><RevenueLineChart orders={dashboard.orders} /></div><div className="dashboard-two-column"><OrderStatus orders={dashboard.orders} /><TopProducts products={dashboard.topProducts} /></div><RecentOrders orders={dashboard.orders} /></>}</div>
}
